...
class Rigid_Loss(nn.Module):
    ...
    def __init__(self, meshes_src):
        ...
        self.undirected_edges_src, self.cot_laplacian_weights = to_undirected(self.edges_src,
                                                                              self.cot_laplacian_weights, meshes_src._V)
        ...
        self.neighborhood_coords_src = rearrange(self.neighborhood_coords_src, '(v d) c -> v d c', v=meshes_src._V,
                                                 d=self.max_degree_src, c=3)
        # gather the weights
        self.neighborhood_weights = self.cot_laplacian_weights[self.undirected_edges_src_eq[1]].view(meshes_src._V,
                                                                                                     self.max_degree_src)
    ...
    def forward(self, new_verts_coords, if_elastic=False):
        ...
        neighborhood_coords_trg = rearrange(neighborhood_coords_trg, '(v d) c -> v d c', v=self.meshes_src._V,
                                            d=self.max_degree_src, c=3)
